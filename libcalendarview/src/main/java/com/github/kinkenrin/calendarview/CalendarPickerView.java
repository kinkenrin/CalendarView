...
public class CalendarPickerView extends ListView {
    ...
    final CalendarMonthView.Listener listener = new CellClickedListener();
...
*/
        ...
        fullDateFormat = new SimpleDateFormat("yyyy-MM-dd", locale);
...
//        displayOnly = false;
        ...
        while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the month.
            ...
            MonthDescriptor month =
                    new MonthDescriptor(monthCounter.get(MONTH), monthCounter.get(YEAR), date, monthNameFormat.format(date));
    ...
    private class CellClickedListener implements CalendarMonthView.Listener {
        @Override
        public void handleClick(MonthCellDescriptor cell) {
            ...
            if (!betweenDates(clickedDate, minCal, maxCal) || !isDateSelectable(clickedDate)) {
                ...
                if (selectedCals.size() == 1) {
                    Date startDate = selectedCals.get(0).getTime();
                    if (((int) ((cell.getDate().getTime() - startDate.getTime()) / (1000 * 3600 * 24))) >= 30) {
                        Toast.makeText(getContext(), "时间跨度只支持最长30天", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            CalendarMonthView monthView = (CalendarMonthView) convertView;
            if (monthView == null) {
                monthView = CalendarMonthView.create(getContext(), parent, inflater, R.drawable.calendar_bg_selector);
//                monthView.setTag(R.id.day_view_adapter_class, dayViewAdapter.getClass());
            }
            monthView.bindData(months.get(position), cells.getValueAtIndex(position), listener);
            return monthView;
        }
    }
    ...
    private boolean beforeToday(Calendar cal, Calendar selectedDate) {
        if (cal.getTime().compareTo(selectedDate.getTime()) < 0) {
            return false;
        }
        return false;
    }
    ...
    public String getCalendarDateStr() {
        ...
        if (selectedDates != null && selectedDates.size() > 0) {
            if (selectedDates.size() == 1) {
                return fullDateFormat.format(selectedDates.get(0).getTime());
            } else {
                return fullDateFormat.format(selectedDates.get(0).getTime()) + " 至 " + fullDateFormat.format(selectedDates.get(selectedDates.size() - 1).getTime());
            }
        }
        return "";
    }
